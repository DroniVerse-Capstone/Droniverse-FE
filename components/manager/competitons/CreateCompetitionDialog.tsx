...
import { useCreateCompetition } from "@/hooks/competitions/useCompetitions";
import { createCompetitionRequestSchema } from "@/validations/competitions/competitions";
...
export default function CreateCompetitionDialog({
  ...
  const t = useTranslations("ManagerCompetitions.createDialog");
  const [form, setForm] = React.useState(DEFAULT_FORM);

  const createCompetitionMutation = useCreateCompetition();

  const isSubmitting = createCompetitionMutation.isPending;

  const normalizedPayload = React.useMemo(() => {
    // Return structured payload based on form data plus clubId
    return {
      clubID: clubId,
  ...
  const formValidation = React.useMemo(() => {
    return createCompetitionRequestSchema.safeParse(normalizedPayload);
  }, [normalizedPayload]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      await createCompetitionMutation.mutateAsync(formValidation.data);
          ...
          <div className="flex-1 overflow-y-auto px-6 py-4 space-y-6">
            <div className="grid gap-4 md:grid-cols-2">
              <div className="space-y-2">
                <Label htmlFor="comp-name-vn">{t("fields.nameVn")}</Label>
                <Input
                  id="comp-name-vn"
                  value={form.nameVN}
                  onChange={(e) => setField("nameVN", e.target.value)}
                  placeholder={t("fields.nameVnPlaceholder")}
                  className="bg-greyscale-850 border-greyscale-700 focus-visible:ring-primary"
                />
              </div>

              <div className="space-y-2">
                <Label htmlFor="comp-name-en">{t("fields.nameEn")}</Label>
                <Input
                  id="comp-name-en"
                  value={form.nameEN}
            ...
            <div className="space-y-2">
              ...
              <div className="grid gap-4 md:grid-cols-2 pt-2">
                <div className="space-y-2">
                  <Label htmlFor="comp-visible-at">{t("fields.visibleAt")}</Label>
                  <Input
                    id="comp-visible-at"
                    type="datetime-local"
                ...
                <div className="space-y-2">
                  <Label htmlFor="comp-reg-start">{t("fields.registrationStartDate")}</Label>
                  <Input
                    id="comp-reg-start"
                    type="datetime-local"
                ...
                <div className="space-y-2">
                  <Label htmlFor="comp-reg-end">{t("fields.registrationEndDate")}</Label>
                  <Input
                    id="comp-reg-end"
                    type="datetime-local"
                ...
                <div className="space-y-2">
                  <Label htmlFor="comp-start">{t("fields.startDate")}</Label>
                  <Input
                    id="comp-start"
                    type="datetime-local"
                ...
                <div className="space-y-2">
                  <Label htmlFor="comp-end">{t("fields.endDate")}</Label>
                  <Input
                    id="comp-end"
                    type="datetime-local"
                    value={form.endDate}
            ...
            <div className="space-y-2">
              <Label>{t("fields.ruleContent")}</Label>
              <QuillEditor
                value={form.ruleContent}
                onChange={(val) => setField("ruleContent", val)}
                placeholder={t("fields.ruleContentPlaceholder")}
                minHeight={250}
              />
            </div>
          </div>
